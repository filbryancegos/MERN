import React, { useState } from 'react'
import { Loader } from '../../components'
import 'react-toastify/dist/ReactToastify.css';
import { useNavigate } from 'react-router-dom';
import { useAuthContext } from '../../context/AuthContext';

function Login() {
  const navigate = useNavigate();
  const { login, dispatch, loading } = useAuthContext()

  const [errors, setErrors] = useState({
    username: '',
    password: '',
  })

  const [fields, setFields] = useState({
    username: '',
    password: '',
  })

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFields(prev => ({
      ...prev,
      [name]: value
    }))
  }

  const handleNotification = (payload, type) => {
    dispatch({
      type: 'IS_NOTIFICATION',
      payload: {
        message: payload,
        isNotified: true,
        type
      },
    })

    setTimeout(() => {
      dispatch({
        type: 'IS_NOTIFICATION',
        payload: {
          message: '',
          isNotified: false,
          type: '',
        },
      })
    }, 2000);
  }

  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await login(fields);
      if (res.data.success) {
        setErrors({})
        dispatch({
          type: 'IS_LOADING',
          payload: false,
        })
        localStorage.setItem('user', JSON.stringify(res.data.user))
        localStorage.setItem('token', JSON.stringify(res.data.token))
        dispatch({
          type: 'LOGIN',
          payload: res.data.user,
        })

        handleNotification(res.data.message, 'success') 
        navigate('/todo')    
      }
    } catch (error) {
      const err = error.response.data
      if (!err.success && err.message) {
        handleNotification(err.message, 'error')
        setErrors({})
      } else if (!err.success && err.errors) {
        setErrors(err.errors ? err.errors : {})
        dispatch({
          type: 'IS_NOTIFICATION',
          payload: {
            message: '',
            isNotified: false,
            type: 'error'
          },
        })
      }
      dispatch({
        type: 'IS_LOADING',
        payload: false,
      })
    }
  }

  return (
    <form onSubmit={handleSubmit}>
      <div className="mt-8 max-w-md m-auto">
      <div className="grid grid-cols-1 gap-6">
        <label className="block">
          <span className={`${errors?.username ? 'text-red-500' : 'text-gray-700'}`}>Username</span>
          <input onChange={handleChange} name='username' value={fields.username} type="text" className="mt-1 block w-full" placeholder="" />
          {errors?.username && <span className="text-red-500 text-base">{errors?.username}</span>}
        </label>
        <label className="block">
          <span className={`${errors?.password ? 'text-red-500' : 'text-gray-700'}`}>Password</span>
          <input onChange={handleChange} name='password' value={fields.password}  type="password" className="mt-1 block w-full"/>
          {errors?.password && <span className="text-red-500 text-base">{errors?.password}</span>}
        </label>           
        <div className="block">
          <div className="mt-2">
            <div>
              <button type='submit' className='bg-blue-400 p-2 text-white w-full flex justify-center items-center gap-2'>
                <span>Submit</span>
                { loading && 
                  <Loader />
                }
              </button>
            </div>
          </div>
        </div>
        <div className="flex justify-end">
          <p>Don't have an account? <span className='text-blue-400'> <span onClick={() => {
            navigate('/register');
          }}>Register</span></span></p>
        </div>
      </div>
    </div>
    </form>
    
  )
}

export default Login